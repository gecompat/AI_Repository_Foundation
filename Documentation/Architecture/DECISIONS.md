...
## DEC-0014 — Foundation upgrades require complete semantic feature applicability assessment

- Status: Accepted
- Date: 2026-08-24
- Context: File-level transfer and semantic merge rules can correctly preserve an older target repository while still missing a newer Foundation improvement if an AI does not infer that the improvement is relevant. Persistent identifier/nomenclature adoption exposed this gap: a target can upgrade Foundation files without the upgrade process actively surfacing the better identity model for future work.
- Decision: Every Foundation upgrade from an older installed ruleset MUST compute the complete feature delta from the source feature catalog. Every feature introduced after the installed version, or materially changed after it, MUST be assessed exactly once against the target repository. Assessment outcomes are `NOT_APPLICABLE`, `ALREADY_EQUIVALENT`, `PROJECT_STRONGER`, `APPLY_DEFAULT`, `RECOMMENDED`, `DECISION_REQUIRED`, or `CONFLICT`. Relevant improvements with `RECOMMENDED`, `DECISION_REQUIRED`, or `CONFLICT` MUST be surfaced explicitly before upgrade completion. Persistent identity uses durable-identifier signals and recommends `ADOPT_FORWARD` when the current convention is compatible but materially weaker; historical migration still requires explicit `MIGRATE_EXPLICIT` authority.
- Rationale: Applicability should be guided by machine-readable feature metadata and repository evidence, not depend on whether a particular AI happens to notice a new policy. This retains project-specific semantic judgment while making omission of new Foundation features observable and testable.
- Alternatives: relying solely on AI reading discipline was rejected as non-deterministic; automatically applying all new Foundation features was rejected because projects may already have equivalent/stronger governance or may require a durable decision; encoding applicability only in prose was rejected because version-delta completeness could not then be checked deterministically.
