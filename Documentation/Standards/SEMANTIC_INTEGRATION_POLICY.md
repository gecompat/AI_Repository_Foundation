...
Installing the Foundation into an existing repository is a semantic integration, not a text-replacement exercise. The Foundation adds a reusable baseline while the target repository remains authoritative for project facts, domain rules, architecture, project-selected policies, validation, implementation, and established identifier history.
...
- `FOUNDATION_REQUIRED_CONFLICT`: the target would weaken or contradict a Foundation `REQUIRED` minimum in a way that materially reduces the protected safety, privacy, integrity, evidence, authorization, or identity property. Resolve before completion.
...
- project-specific validation/model/privacy/license/identifier contracts remain preserved;
