...
class InstallationModelTests(unittest.TestCase):
    ...
    def test_core_semantic_identity_registration_registry_upgrade_and_eol_material_is_installed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp)
            self.assertEqual(self.install(target), 0)
            ...
            self.assertEqual(catalog["ruleset_version"], "1.6.1")
    ...
    def test_manifest_sources_exist_targets_unique_and_version_is_v1_6_1(self) -> None:
        ...
        self.assertEqual(self.manifest["ruleset_version"], "1.6.1")
