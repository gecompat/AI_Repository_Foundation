...
The optional `model-router` capability preserves `foundation-model-router/v1` and adds the compatible `foundation-model-router/v2` facade. Select it only when the target wants the local Python reference implementation and has authorized its runtime/provider boundaries. It supplies a provider-neutral router core, isolated provider fragments, Ollama Cloud discovery, CLI, local stdio MCP server, shell-free launcher, and expiring snapshots. Its v1 and v2 request, decision, catalog, fragment, snapshot, and profile schemas are core so another implementation can use the same contracts.
...
The core `foundation-ai-work/v1` schemas and `AI_WORK_ORCHESTRATION_POLICY.md` apply without any executable AI component. Select `ai-work`, `ai-runtime-adapters`, `ai-executor`, `ai-provisioning`, `ai-client-integration`, and `ai-orchestrator` only when their optional reference behavior is useful. Manifest dependencies install required companion capability code but grant no runtime authority, configuration, credentials, data transfer, spend, or execution permission.

The optional `ai-orchestrator` composes live failure-isolated runtime catalogs, fresh external source-backed model evidence, router v2, content-handle invocation, deterministic validation, bounded fallback, and a content-free report. Selecting it also selects its declared `ai-work`, `ai-runtime-adapters`, and `model-router` dependencies. It is not a replacement for the generic `ai-executor`: the executor runs exact `foundation-ai-work/v1` plans, while the orchestrator is a narrower model-routing facade. Without configuration or adequate evidence it must return a manual/unavailable result and must not infer quality, price, capability, context, aliasing, or actual execution from catalog presence.
...
6. Select `core`, requested adapters, and only explicitly requested/project-selected optional capabilities. In particular, select `rule-context-cache` only for local deterministic cache planning; `model-router` only for decision routing; `ai-runtime-adapters` only for configured runtime discovery/invocation; `ai-executor` only for exact-plan execution; `ai-provisioning` only for bounded host preparation; `ai-client-integration` only for governed client configuration/dispatch; and `ai-orchestrator` only for the complete evidence-backed route/invoke/validate facade. Preserve every capability's separate authority and external-state boundary.
